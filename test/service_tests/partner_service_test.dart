  ...
  group('Partners', () {
    ...
    test('create referral customer', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'create_referral_customer',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode(); // Test mode is set to partner key

      final params = Fixtures.createReferralCustomer;

      final referralCustomer =
          await client.partners.createReferralCustomer(params);
    ...
    test('retrieve referral customer', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'retrieve referral customer',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      ...
      final retrievedReferralCustomer =
          await client.partners.retrieveReferralCustomer(referralCustomer.id);
    ...
    test('list referral customers', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'list referral customers',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
      ...
      final referralCustomerCollection =
          await client.partners.listReferralCustomers(parameters: params);

      expect(referralCustomerCollection, isNotNull);
      expect(referralCustomerCollection.referralCustomers, isNotNull);
      expect(
          referralCustomerCollection.referralCustomers!.length <=
              Fixtures.pageSize,
          true);
      for (ReferralCustomer referralCustomer
          in referralCustomerCollection.referralCustomers!) {
    ...
    test('next page of referral customers', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'next_page_of_referral_customers',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
      ...
      final nextPage = await client.partners.getNextPageOfReferralCustomers(
    ...
    test('update referral customer', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'update_referral_customer',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
      ...
      final success = await client.partners
          .updateReferralCustomerEmail(referralCustomer.id, updateParams);

      expect(success, isTrue);

      final updatedReferralCustomer =
          await client.partners.retrieveReferralCustomer(referralCustomer.id);
    ...
    test('add credit card to referral customer', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'add_credit_card_to_referral_customer',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();

      final referralCustomer = await client.partners
          .createReferralCustomer(Fixtures.createReferralCustomer);
      ...
      expect(
          () async => await client.partners.addCreditCardToReferralCustomer(
              referralCustomerApiKey, creditCardParams),
          throwsA(isA<ResourceNotFoundException>()));
    });

    test('add existing Stripe payment method to referral customer', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'add_existing_stripe_payment_method_to_referral_customer',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
      ...
      stripeParams.stripePaymentMethodId =
          "not-a-real-stripe-payment-method-id";

      try {
        final paymentMethod = await client.partners
            .addExistingStripePaymentMethodToReferralCustomer(
                referralCustomerApiKey, stripeParams);
    ...
    test('issue refund for referral customer by amount', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'issue_refund_for_referral_customer_by_amount',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
      ...
      try {
        final refund = await client.partners
            .issueRefundToReferralCustomerAccount(
                referralCustomerApiKey, refundParams);
        fail('Exception not thrown');
      } on ApiInvalidRequestException catch (e) {
        // Data being sent is not valid, so we expect an error
    ...
    test('issue refund for referral customer by payment log id', () async {
      Client client = TestUtils.setUpVCRClient(
          "partners", 'issue_refund_for_referral_customer_by_payment_log_id',
          overrideTestApiKey: getApiKey(ApiKeyEnum.partner));
      client.enableTestMode();
